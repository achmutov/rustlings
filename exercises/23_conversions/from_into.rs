...
impl From<&str> for Person {
    fn from(s: &str) -> Self {
        let [name_raw, age_raw] = s.split(",").collect::<Vec<_>>()[..] else {return Person::default()};
        match (name_raw, age_raw.parse::<u8>()) {
            (name, Ok(age)) if !name.is_empty() => Person { name: name.to_string(), age },
            _ => Person::default(),
        }
    }
}
